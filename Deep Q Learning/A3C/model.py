...
class ActorCritic:
  ...
  '''
  def __init__(self, env, n_timesteps, mode='train'):
  ...
  def feed_forward(self, ob):
    ...
    if not self.mode == 'train':
      ...
      num_actions = len(qvals[0][-1]) #[0][-1] = last observation
      action = np.random.choice(num_actions, 1, p=np.around(qvals[0][-1], decimals=5))[0]
